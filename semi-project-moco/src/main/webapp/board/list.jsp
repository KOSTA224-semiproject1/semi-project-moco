...
<div class="container center-block pt-5 mt-3" style='width: 650px;'>
	<form action="SearchPostListPageController.do" onkeydown="onEnter()">
		...
		<div class="input-group mt-3 mb-3">
			...
			<input type="text" class="form-control" name="q" value="${param.q}" placeholder="검색어를 입력하세요" required="required">
	...
	<div>
		...
		<div style="display: inline-block; float: right;">
			<button type="button" class="btn btn-primary"
				onclick="moveToMainPage()">카테고리</button>
			<button type="button" class="btn btn-primary"
				onclick="moveToWritePage('${requestScope.languageCode}')">글쓰기</button>
		</div>
	</div>
	...
	<ul class="pagination justify-content-center" style="margin: 20px 0">
		<c:if test="${pagingBean.previousPageGroup}">
			<li class="page-item"><a class="page-link"
				href="ListPageController.do?pageNo=${pagingBean.startPageOfPageGroup-1}">Previous</a></li>
		</c:if>
		...
		<c:if test="${pagingBean.nextPageGroup}">
			<li class="page-item"><a class="page-link"
				href="ListPageController.do?pageNo=${pagingBean.endPageOfPageGroup+1}">Next</a></li>
		</c:if>
	</ul>
</div>