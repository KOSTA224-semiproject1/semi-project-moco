...
public class BoardDAO {
	...
	public PostVO getPostingByPostNo(String postNo) throws SQLException {
		...
		try {
			...
			StringBuilder sql = new StringBuilder();
			sql.append(
					"select b.post_no, b.post_title, b.post_content, b.post_code, to_char(b.post_regdate, 'yyyy-mm-dd') as post_regdate, b.hits, m.nickname, m.email, b.language_code ");
	...
	public ArrayList<PostVO> getMemberPosts(String email) throws SQLException {
		...
		try {
			...
			StringBuilder sql = new StringBuilder();
			sql.append(
					"select v.post_no, v.post_title, to_char(v.post_regdate, 'yyyy-mm-dd') as post_regdate, v.hits, l.language as lang ");
			sql.append("from moco_qna_board v, moco_service_language l ");
			sql.append("where l.language_code=v.language_code ");
			sql.append("and v.email=? ");
			...
			while (rs.next()) {
				...
				LanguageVO lvo = new LanguageVO();
				lvo.setLanguage(rs.getString("lang"));
				pvo.setLvo(lvo);
				list.add(pvo);
			}
		} finally {
